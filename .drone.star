...
def linux(ctx, arch):
    if ctx.build.event == "tag":
        build = [
            'go build -v -ldflags "-X main.version=%s" -a -tags netgo -o release/linux/%s/drone-cancel-previous-builds ./cmd/drone-cancel-previous-builds' % (ctx.build.ref.replace("refs/tags/v", ""), arch),
        ]
    else:
        build = [
            'go build -v -ldflags "-X main.version=%s" -a -tags netgo -o release/linux/%s/drone-cancel-previous-builds ./cmd/drone-cancel-previous-builds' % (ctx.build.commit[0:8], arch),
        ]
    ...
    "steps": [
        {
            ...
            "environment": {
                ...
                "PUSHRM_SHORT": "Drone plugin to cancel previous pipelines",
